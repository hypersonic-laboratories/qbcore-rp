...
local intervalTimer
local apartmentMarkers = {}
local OwnedApartmentName = nil

local COLOR_OWNED = LinearColor(0.55, 0.85, 1.0, 1.0)
local COLOR_UNOWNED = LinearColor(0.1, 0.3, 0.65, 1.0)

local function ClearApartmentMarkers()
    for _, id in ipairs(apartmentMarkers) do
        exports['qb-hud']:RemoveMarker(id)
    end
    apartmentMarkers = {}
end

local function CreateApartmentMarkers()
    ClearApartmentMarkers()
    for id, apartment in pairs(Apartments.Locations) do
        local markerId = exports['qb-hud']:AddMarker(Vector(apartment.coords[1], apartment.coords[2], apartment.coords[3]), {
        ...
        if markerId then
            apartmentMarkers[#apartmentMarkers + 1] = markerId
        end
    end
end
...
local function SetInApartmentTargets(apartmentName)
    ...
    local entrancePos = Vector(Apartments.Locations[apartmentName].coords[1] - POIOffsets.exit.x, Apartments.Locations[apartmentName].coords[2] - POIOffsets.exit.y, Apartments.Locations[apartmentName].coords[3] + CurrentOffset + POIOffsets.exit.z)
...
local function SetClosestApartment()
    ...
    for id in pairs(Apartments.Locations) do
        local apartmentPos = Vector(Apartments.Locations[id].coords[1], Apartments.Locations[id].coords[2], Apartments.Locations[id].coords[3])
...
RegisterClientEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    TriggerCallback('GetOwnedApartment', function(result)
        if type(result) == 'string' then
            OwnedApartmentName = result
        elseif type(result) == 'table' then
            OwnedApartmentName = result.name or result.apartment or result.apartmentName
        else
            OwnedApartmentName = nil
        end
...
RegisterClientEvent('qb-apartments:client:LeaveApartment', function()
    exports['qb-clothing']:SaveCurrentSkin()
...
RegisterClientEvent('qb-apartments:client:UpdateApartment', function()
    IsOwned = true
    OwnedApartmentName = ClosestApartment
    DeleteApartmentsEntranceTargets()
    DeleteInApartmentTargets()
    CreateApartmentMarkers()
end)
...
if HConsole then
    HConsole:RegisterCommand('getoffset', 'Apartment Offset', nil, {
        HWorld,
        function()
            if not InApartment then
                return
            ...
            local spawnPos = Vector(Apartments.Locations[ClosestApartment].coords[1], Apartments.Locations[ClosestApartment].coords[2], Apartments.Locations[ClosestApartment].coords[3] + CurrentOffset)
            ...
            local offset = ('{ x = %.2f, y = %.2f, z = %.2f }'):format(spawnPos.X - pos.X, spawnPos.Y - pos.Y, pos.Z - spawnPos.Z)
