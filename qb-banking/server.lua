...
RegisterCallback('openATM', function(source)
    ...
    local bankCards = Player.GetItemsByName('bank_card')
    if not bankCards then
        return TriggerClientEvent(source, 'QBCore:Notify', Lang.t('error.card'), 'error')
    end
...
RegisterCallback('withdraw', function(source, data)
    ...
    if accountName == 'checking' then
        ...
        if accountBalance < withdrawAmount then
            return { success = false, message = Lang.t('error.money') }
        end
    ...
    if Accounts[accountName] then
        ...
        if Accounts[accountName].account_type == 'job' and job.name ~= accountName and not job.isboss then
            return { success = false, message = Lang.t('error.access') }
        end
        ...
        if not CreateBankStatement(source, accountName, withdrawAmount, reason, 'withdraw', Accounts[accountName].account_type) then
            return { success = false, message = Lang.t('error.error') }
        end
        return { success = true, message = Lang.t('success.withdraw') }
    end
...
RegisterCallback('externalTransfer', function(source, data)
    ...
    if fromAccountName == 'checking' then
        ...
        if not CreateBankStatement(toPlayer.PlayerData.source, 'checking', transferAmount, reason, 'deposit', 'player') then
            return { success = false, message = Lang.t('error.error') }
        end
        return { success = true, message = Lang.t('success.transfer') }
    else
...
RegisterCallback('orderCard', function(source, data)
    ...
    if not pinNumber then
        return { success = false, message = Lang.t('error.pin') }
    end
    local info = {
        citizenid = citizenid,
    ...
    if not Player.AddItem('bank_card', 1, false, info) then
        return { success = false, message = Lang.t('error.error') }
    end
...
RegisterCallback('openAccount', function(source, data)
    ...
    if GetNumberOfAccounts(citizenid) >= Config.maxAccounts then
        return { success = false, message = Lang.t('error.accounts') }
    end
    ...
    if not CreateBankStatement(source, 'checking', initialAmount, 'Initial deposit for ' .. accountName, 'withdraw', 'player') then
        return { success = false, message = Lang.t('error.error') }
    end
...
RegisterCallback('addUser', function(source, cb, data)
    ...
    local accountName = data.accountName
    if not Accounts[accountName] then
        return { success = false, message = Lang.t('error.account') }
    end
    if Accounts[accountName].citizenid ~= citizenid then
        return { success = false, message = Lang.t('error.access') }
    end
    ...
    for _, cid in ipairs(users) do
        if cid == userToAdd then
            return { success = false, message = Lang.t('error.user') }
        end
    end
...
RegisterCallback('removeUser', function(source, cb, data)
    ...
    if not userFound then
        return { success = false, message = Lang.t('error.noUser') }
    end
    local usersData = JSON.stringify(users)
    Accounts[accountName].users = usersData
    local result = exports['qb-core']:DatabaseAction('Execute', 'UPDATE bank_accounts SET users = ? WHERE account_name = ? AND citizenid = ?', { usersData, accountName, citizenid })
