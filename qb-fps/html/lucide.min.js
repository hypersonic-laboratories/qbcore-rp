    ...
    const n = { xmlns: "http://www.w3.org/2000/svg", width: 24, height: 24, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", "stroke-width": 2, "stroke-linecap": "round", "stroke-linejoin": "round" },
        ...
        Wu = (...t) =>
            t
                ...
                .join(" ")
                .trim(),
        Eu = (t) => t.replace(/^([A-Z])|[\s-_]+(\w)/g, (h, d, c) => (c ? c.toUpperCase() : d.toLowerCase())),
        Iu = (t) => {
            const h = Eu(t);
...
lucide.createIcons({icons});\``);
        ...
        if ((Array.from(c.querySelectorAll(`[${h}]`)).forEach((p) => Ba(p, { nameAttr: h, icons: t, attrs: d })), M && Array.from(c.querySelectorAll("template")).forEach((p) => Uu({ icons: t, nameAttr: h, attrs: d, root: p.content, inTemplates: M })), h === "data-lucide")) {
            const p = c.querySelectorAll("[icon-name]");
            p.length > 0 && (console.warn("[Lucide] Some icons were found with the now deprecated icon-name attribute. These will still be replaced for backwards compatibility, but will no longer be supported in v1.0 and you should switch to data-lucide"), Array.from(p).forEach((Sa) => Ba(Sa, { nameAttr: "icon-name", icons: t, attrs: d })));
        }
    };
